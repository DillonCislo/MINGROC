...
void mexFunction( int nlhs, mxArray *plhs[],
	...
	if ( nrhs != 5 ) {
		mexErrMsgIdAndTxt( "MATLAB:isotropic_remeshing:nargin",
				"ISOTROPIC_REMESHING requires five input arguments." );
	} else if ( nlhs != 4 ) {
	...
	// Determine if constraints should be protected (i.e. boundary vertices)
	bool protectConstraints = *mxGetLogicals( prhs[4] );
	...
	PMP::isotropic_remeshing(
			...
			.protect_constraints(protectConstraints) // i.e. protect border
			);
